...
def abrir_nova_janela():
    nova_janela = ttk.Toplevel(root)
    nova_janela.title("Fazer Requisição")
    nova_janela.geometry("900x455")
    ...
    label_codigo_desc = ttk.Label(nova_janela, text="Código e Descrição do Item:")
    label_codigo_desc.pack(pady=5)

    frame_combobox = ttk.Frame(nova_janela)
    ...
    label_info_combobox = ttk.Label(frame_combobox, text="Clique no ícone no canto direito do campo para selecionar o item.")
    label_info_combobox.pack(side=tk.LEFT, padx=5)

    label_quantidade = ttk.Label(nova_janela, text="QUANTIDADE EM ESTOQUE:", font=("TkDefaultFont", 10, "bold"))
    label_quantidade.pack(pady=5)

    label_quantidade_solicitada = ttk.Label(nova_janela, text="Quantidade Solicitada:")
    label_quantidade_solicitada.pack(pady=5)

    entry_quantidade = ttk.Entry(nova_janela, textvariable=pR2_Quant, validate="key")
    ...
    label_solicitante = ttk.Label(nova_janela, text="Solicitante:")
    label_solicitante.pack(pady=5)
    entry_solicitante = ttk.Entry(nova_janela, textvariable=solicitante_var, width=50)
    entry_solicitante.pack(pady=5)

    label_motivo_os = ttk.Label(nova_janela, text="Motivo/OS:")
    label_motivo_os.pack(pady=5)
    entry_motivo_os = ttk.Entry(nova_janela, textvariable=motivo_os_var, width=50)
    entry_motivo_os.pack(pady=5)

    label_data_entrega = ttk.Label(nova_janela, text="Data de Entrega (dd/mm/aaaa):")
    label_data_entrega.pack(pady=5)
    entry_data_entrega = ttk.Entry(nova_janela, textvariable=data_entrega, width=20)
    ...
    def enviar_dados():
        ...
        if not (descricao and quantidade and codigo_produto and solicitante and motivo_os and data_entrega_selecionada):
    ...
    button_enviar_dados = ttk.Button(nova_janela, text="Enviar", command=enviar_dados)
    button_enviar_dados.pack(pady=20)
...
def enviar_email(codigo_produto, descricao, quantidade, solicitante, motivo_os, data_entrega):
    ...
    email.HTMLBody = f"""
    <p>Automação python da requisição de estoque.</p>
...
# Configuração da interface principal com o tema
...
progress_label = ttk.Label(root, text="Progresso: 0%")
progress_label.pack(pady=10)

botao_iniciar = ttk.Button(root, text="Iniciar Consulta", command=iniciar_consulta)
botao_iniciar.pack(pady=20)

botao_fazer_requisicao = ttk.Button(root, text="Fazer Requisição", state=tk.DISABLED, command=abrir_nova_janela)
