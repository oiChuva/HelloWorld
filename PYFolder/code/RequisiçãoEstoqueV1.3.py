...
def consultar_multiplos_estoques(data_posicao):
    ...
    if todas_respostas:
        ...
        if not df_estoques_concatenados.empty:
            ...
            root.after(0, consulta_concluida, nome_arquivo)
        else:
            root.after(0, consulta_vazia)
    else:
        root.after(0, consulta_vazia)
...
def iniciar_consulta():
    ...
    threading.Thread(target=consultar_multiplos_estoques, args=(data_posicao,)).start()

def update_progress(progress):
    progress_bar['value'] = progress
    progress_label.config(text=f'Progresso: {int(progress)}%')

def consulta_concluida(nome_arquivo):
...
def abrir_nova_janela():
    ...
    def enviar_dados():
        ...
        if not validar_data(data_entrega_selecionada):
    ...
    label_data_entrega = tk.Label(nova_janela, text="Versão 1.30")
    label_data_entrega.pack(pady=5)
...
def enviar_email(codigo_produto, descricao, quantidade, solicitante, motivo_os, data_entrega):
    ...
    email.HTMLBody = f"""
    <p>Este é um envio de email para a automação python da requisição de estoque.</p>
